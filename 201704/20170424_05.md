...
```  
一张表更新另一张表
update,delete批量操作，Join不是一一对应的，更新目标可能会随机匹配
update set info=t1.info,crt_time=t1.crt_time from t1,t2 where t1,t2 where (t.id=t1.id) and t1.id=t2.id;
...
```
用一张表更新另一张表
delete from using t1 where t.id=t1.id;
