...
##wangji：PG crash，打印coredump时使用
##kernel.core_pattern=/data1/corefiles/core_%e_%u_%t_%s.%p
...
##wangji：与iptables相关，最大会话数超过他们会丢包
```  
...
```  
##wangji
系统脏页到达这个值，OS后台刷脏页调度进程pdflush自动将dirty_expire_centisecs/100秒前的脏页刷到磁盘
默认=10%，大内存机器建议调整为直接指定为多少字节
vm.dirty_background_bytes = 4096000000    

比这个值老的脏页，将被刷到磁盘，6000表示60秒
vm.dirty_expire_centisecs = 6000    

若系统进程刷脏页太慢，使得系统脏页超过内存80时，则用户进程若有写磁盘的操作（eg：fsync，fdatasync等），则需要主动把系统脏页刷出；
有效防止用户进程刷脏页，在单机多实例，且使用CGGROUP限制单实例IOPS的情况下十分有效；
vm.dirty_ratio = 80    

pdflush后台刷脏页进程的唤醒间隔，50表示0.5秒
...
watermark[WMARK_MIN] value for each // zone in the system.    
...
##当vm.overcommit_memory=2时，用于参与计算允许指派的内存大小
vm.overcommit_ratio = 90    
.  
vm.overcommit_memory = 0 时 vm.overcommit_ratio可以不设置   
##wangji：在分配内存时，运行少量malloc，若设置为1，则认为总是有足够的内存，内存较少的测试环境可以使用1
```  
...
##wangji：建议shared buffer设置超过32GB时候使用大页，页大小：/proc/meminfo Hugepagesize
```  
