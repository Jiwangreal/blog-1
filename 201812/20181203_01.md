...
# 监听    
##wangji：
#listen_addresses = '0.0.0.0'表示任意IPv4地址
#listen_addresses = '*'表示任意IPv4、Ipv6地址
...
# max_connections 公式：物理内存(GB)*2000*(1/4)/5     
...
#业务和数据库放在一起时，可以这么去干，.代表$PGDATA目录
# $PGDATA, /tmp中 创建unix socket监听    
#[postgres@node4 data]$ netstat -anp|grep post
#unix  2      [ ACC ]     STREAM     LISTENING     26669741 3841670/postgres     ./.s.PGSQL.1921
unix_socket_directories = '., /tmp'        # comma-separated list of directories      
                                        # (change requires restart)      
#unix_socket_group = ''                 # (change requires restart)      

##wangji：unix_socket_permissions =0777其他用户也可以连接
...
##wangji:max_prepared_transactions 公式： max_prepared_transactions=max_connections 
...
##wangji:max_parallel_maintenance_workers 表示多少个进程并行创建索引
# 创建索引时使用的内存空间。    
# maintenance_work_mem 公式： min( 8G, (主机内存*1/8)/max_parallel_maintenance_workers )        
maintenance_work_mem = 2GB              # min 1MB      

##wangji:回收垃圾的时候表扫描多少遍的问题，值过小，一张表的垃圾很多，会导致扫表很多表，导致垃圾回收性能很差
...
##wangji:表示整个数据库集群同时进行并行计算的workers进程
...
##wangji:写REDO时，当一个检查点做完之后，一个数据块第一次被变更，要不要把整个数据块的内容写到REDO里面。
##若文件系统能提供8K的原子写，则关闭，REDO可以写少一点，特别是update操作的多的话，性能会好很多
...
##wangji： REDO日志大于48G或者在max_wal_size 和min_wal_size 之间会触发checkpoint
...
##wangji：越小，表示在很短的时间内（checkpoint_timeout*0.2或者24GB*0.2）刷完shared buffer脏页
...
##wangji：脏页是256KB时，调用flush（异步写接口）
...
#wangji：     是否需要流复制建备库，使用流复制建立归档、备份等  
...
# These settings are ignored on a publisher.      
 ...
 ##wangji：最多订阅多少张表，最多开启多少逻辑订阅的客户端
...
##wangji：SSD的随机IO和顺序IO配置为1：1，否则使用bitmap scan，他比hash can比有recheck,性能差
...
##wangji：优化器倾向于使用索引扫描还是全局扫描呢？会有一个算子，提供一个参考值
...
#wangji：刷了多少脏页，花了多长时间，异步写、同步写花的时间
# 记录检查点的详细统计信息    
log_checkpoints = on       
...
#wangji:一个垃圾回收占用的内存：autovacuum_work_mem
...
##wangji：SQL超时时间，因为你因为锁而等待，别人也访问该锁，因为锁冲突别人也等待
# 可以用来防止雪崩，但是不建议全局设置      
...
\           # min( 8G, (规格内存*1/8)/autovacuum_max_workers )     
...
##wangji：local表示通过unix socket，all表示所有数据库
...
##wangji：replication表示流复制协议，standby，备份等
